-- ============================================================
-- ACTUALIZACIÓN DE PORTAL DEL ESTUDIANTE: SOPORTE PROFESORES (PLAN ACADEMIA)
-- ============================================================
-- Modifica get_student_portal_data para retornar el professor_id y el professor_name
...
BEGIN
  ...
  -- 2. Obtener Feedback (con extension_proposal si existe)
  SELECT jsonb_agg(
    jsonb_build_object(
      'id', id,
      'user_id', user_id,
  ...
  -- 3.5 Obtener TODAS las Citas con JOIN a Profesores (Plan Academia)
  SELECT jsonb_agg(
    jsonb_build_object(
      'id', a.id,
      'user_id', a.user_id,
  ...
  INTO v_appointments
  FROM appointments a
  LEFT JOIN professors prof ON a.professor_id = prof.id
