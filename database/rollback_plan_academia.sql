-- database/rollback_plan_academia.sql
-- ARCHIVO DE REVERSIÓN (ROLLBACK) PARA EL PLAN ACADEMIA
-- Ejecuta este script si deseas cancelar la implementación y devolver tu base de datos a su estado original.

-- 1. Eliminar la columna `professor_id` de las tablas de horarios/disponibilidad reales
...
-- 3. Eliminar la tabla puente (servicios - profesores)
-- Usamos CASCADE por si hay dependencias adicionales
DROP TABLE IF EXISTS service_professors CASCADE;

-- 4. Eliminar la tabla de profesores
-- Al hacer un DROP CASCADE de la tabla, también se eliminan automáticamente:
-- - Las políticas de seguridad (RLS) asociadas a esta tabla.
-- - El trigger `set_timestamp_professors` que insertamos en ella.
DROP TABLE IF EXISTS professors CASCADE;

-- 5. Restaurar la función original de reservas
-- Esto devuelve crear_public_booking a su versión original sin el p_professor_id
